(ns everlaw-core.main
  "Kotodama WASM entrypoint for the Everlaw_core clean-room actor (L4) — Clojure port.
  ...
  facts (`everlaw_core.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "everlaw_core")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"   :plural "matters"   :id-prefix "everlawc_mat"
    :fields [:clientId :name :status :openedAt]      :required [:name :status]
    :coerce {}                                :refs {}}
   {:entity "Document" :plural "documents" :id-prefix "everlawc_doc"
    :fields [:matterId :custodianId :contentRef :reviewStatus] :required [:reviewStatus]
    :coerce {}                                :refs {:matterId "Matter" :custodianId "Custodian"}}
   {:entity "Custodian" :plural "custodians" :id-prefix "everlawc_cus"
    :fields [:matterId :name :email]                  :required [:name :email]
    :coerce {}                                :refs {:matterId "Matter"}}
   {:entity "Review" :plural "reviews" :id-prefix "everlawc_rev"
    :fields [:documentId :reviewerId :coding]        :required [:coding]
    :coerce {}                                :refs {:documentId "Document"}}
   {:entity "ProductionSet" :plural "productionsets" :id-prefix "everlawc_pro"
    :fields [:matterId :name :documentCount]         :required [:name :documentCount]
    :coerce {:documentCount :int}            :refs {:matterId "Matter"}}
   {:entity "LegalHold" :plural "legalholds" :id-prefix "everlawc_leg"
    :fields [:matterId :custodianId :status :issuedAt] :required [:status :issuedAt]
    :coerce {}                                :refs {:matterId "Matter" :custodianId "Custodian"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"everlaw_core.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "everlaw_core-compat" :tier tier :entities entities} 200])
...
(defn start! [] :everlaw_core-compat/ready)
